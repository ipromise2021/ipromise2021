...
内容以 [官方最佳实践](https://code.claude.com/docs/en/best-practices) 为主线，穿插 Claude Code 之父 **Boris Cherny**  在 X 上分享的真实工作流（标注为 **Boris Pro Tip** ），并从团队实践中提取实用模板。
...
![](https://pica.zhimg.com/v2-82b8fcfd8857b32ea2e4a60edfea9558_r.jpg)
...
![](https://pic4.zhimg.com/v2-191d1b625e60f5cedde58ebcf1f983cd_r.jpg)

## 1.3 Claude Code 功能全景图 🟢

经过一年的高速迭代（2025.02 → 2026.02），Claude Code 已从一个终端工具演进为完整的 AI 编程平台。下图展示了截至 2026 年 2 月的六大功能支柱：

![](https://pic1.zhimg.com/v2-261a77a62537b5fe013439c395e44718_r.jpg)
...
![](https://pic2.zhimg.com/v2-ad043e0795f9808265dede985d7b3a67_r.jpg)
...
首次运行会提示登录，支持以下方式：
- **Claude Max/Pro 订阅** ：通过 [http://claude.com](http://claude.com) 登录
...
![](https://picx.zhimg.com/v2-76dda6d35af8178d7183526d0679eff9_r.jpg)
...
![](https://pic1.zhimg.com/v2-0603d8e4226a2eac91c70b95be420ff4_r.jpg)
...
![](https://pic1.zhimg.com/v2-85fc28eec59d21923715079c0e14a29e_r.jpg)
...
![](https://pic4.zhimg.com/v2-23190052a556c1b967e113702a7ac1fd_r.jpg)
...
![](https://pic3.zhimg.com/v2-ca28102662c446c60b36040c8a8470ea_r.jpg)

注意弹窗上方有 **Tab 标签页** ，可以切换不同维度的问题：

![](https://pica.zhimg.com/v2-e5d4b94706337766fc62947c6b8987ac_r.jpg)

选择完所有选项后，点击 **Submit answers**  继续：

![](https://pic1.zhimg.com/v2-2624097914b05b0fbb5d30c1290f23e2_r.jpg)

然后 Claude 会继续追问其他维度的问题——真的像苏格拉底一样，把你脑海中的想法逐步显化：

![](https://pic4.zhimg.com/v2-ca9119dfcfd53e04c531759926851777_r.jpg)
...
| ---- | ------------------ | ------------------------------------------ | -------------- |
| 存储位置 | 项目根目录 / ~/.claude/ | ~/.claude/projects/&lt;project&gt;/memory/ | 会话内存（不持久化）     |
...
| 查看记忆目录         | 显示 ~/.claude/projects/&lt;project&gt;/memory/ 的完整路径和文件列表 |

> 💡 **调试技巧** ：当 Claude 的行为不符合预期时，先运行 `/memory` 查看加载了哪些文件——问题可能出在过时的自动记忆覆盖了 CLAUDE.md 的规则。

![](https://pic1.zhimg.com/v2-185b4aa0aa97e257817e90a857f9fdfe_r.jpg)
...
![](https://picx.zhimg.com/v2-95e55492d8a00f1dd6ea0e0b609ea5e7_r.jpg)
...
> **异步事件** （不在主流程中，随时可能触发）：`Notification`、`ConfigChange` **独立事件** （Setup/Teardown）：`WorktreeCreate`、`WorktreeRemove`

### Hook 如何解析（以 PreToolUse 为例）

假设你配置了一个 PreToolUse Hook 来阻止危险的 shell 命令。当 Claude 决定执行 `Bash "rm -rf /tmp/build"` 时，以下是完整的解析流程：

![](https://pic1.zhimg.com/v2-7115052e2bf359794fa4ad50232e73b4_r.jpg)
...
![](https://picx.zhimg.com/v2-85680ff507852a03e2c3a8d72648406b_r.jpg)
...
> `/simplify` 不替代 linter 或团队 code review，它是**补充层** ——关注更高层次的设计模式和代码组织，而非格式问题。
...
- 可以通过 Hook 将权限请求路由到 Opus 审批（详见 [Hooks 文档](https://code.claude.com/docs/en/hooks#permissionrequest)）
...
![](https://pic3.zhimg.com/v2-f9d03da5ebdb2ed666f6b0fced83fd88_r.jpg)
...
![](https://pic4.zhimg.com/v2-80e6ee8a61d4960c012dee88fca9ae4b_r.jpg)
...
![](https://pic4.zhimg.com/v2-0299db86ddbde79de8ff32a6fc1b6937_r.jpg)
...
![](https://pic4.zhimg.com/v2-fcfa14d98416327c49cc660f6a54743f_r.jpg)
...
![](https://pica.zhimg.com/v2-4a828e29baaebf2669fa3ae5e7bfdc06_r.jpg)

![](https://pic1.zhimg.com/v2-2ff72b38f613c7c075a0d4a4a04dafe8_r.jpg)
...
![](https://pic1.zhimg.com/v2-c3fdb53f733d9deb39cab86a4ec36a6e_r.jpg)
...
![](https://pic1.zhimg.com/v2-86a3259de1acfdf584c2f6f7c9743362_r.jpg)
...
| Bash 输出截断    | 尾部 30 行          | `npm test 2&gt;&amp;1 \ |
...
![](https://pica.zhimg.com/v2-18846762ffbe7aac5b9f2165376c8c9e_r.jpg)
...
> 💡 **先试 /batch** ：大多数批量迁移场景（框架升级、API 替换、全局重构）可直接使用内置的 `/batch` 技能（见 6.6），它会自动完成任务拆分、并行执行和 PR 生成。下面的手动 Fan-out 适合需要**自定义任务拆分逻辑** 或**特殊并行策略** 的高级场景。
...
| Shipping Faster with Worktrees     | <a href="https://link.zhihu.com/?target=http%3A//incident.io" class=" external" target="_blank" rel="nofollow noreferrer"><span class="invisible">http://</span><span class="visible">incident.io</span><span class="invisible"></span></a> 的 worktree 实战 |
...
![](https://pic4.zhimg.com/v2-7f793cba890228529c724acdea8a9ed5_r.jpg)
